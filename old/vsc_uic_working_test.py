#!/usr/bin/env python3
"""
Test script for VSC_UIC_Working - a UIC VSC with angle tracking that actually works
"""

import numpy as np
import matplotlib.pyplot as plt
from src.dyn_models.vsc_uic_working import VSC_UIC_Working
from src.solvers import EulerDAE

def main():
    """Test VSC_UIC_Working using DAEModel framework with UIC-style control"""
    print("Testing VSC_UIC_Working with UIC-style control...")
    print("=" * 50)
    ...
    # Create VSC
    vsc = VSC_UIC_Working(sys_par=sys_params, **test_params)
    ...
    plt.title(f'VSC_UIC_Working Internal Voltage Components (UIC-style control, P_ref={vsc._input_values["p_ref"].item():.1f}, Q_ref={vsc._input_values["q_ref"].item():.1f}, Ki={vsc.par["Ki"][0]:.1f})')
    plt.legend()
    plt.grid(True)
    ...
    plot_filename = os.path.join(testing_folder, "vsc_uic_working_response.png")
