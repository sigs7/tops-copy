...
if __name__ == '__main__':
    ...
    while t < t_end:
        ...
        sc_bus_idx = ps.vsc['UIC'].bus_idx_red['terminal'][0]
    ...
    ax[0].plot(t_stored, result.xs(key='vix', axis='columns', level=1))
    ax[0].plot(t_stored, result.xs(key='viy', axis='columns', level=1))
    ax[1].plot(t_stored, np.array(I_stored), label='I_gen')
